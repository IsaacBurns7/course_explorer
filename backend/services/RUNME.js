...
function mergeData(existingData, newData) {
    for (const [courseKey, courseVal] of Object.entries(newData)) {
        if (!existingData[courseKey]) {
            ...
            // recalculate averageGPA
            let totalGPA = 0;
            let totalSections = 0;
            for (const sectKey of Object.keys(existingData[courseKey].sections)) {
                totalGPA += existingData[courseKey].sections[sectKey].gpa || 0;
                totalSections++;
            }
            existingData[courseKey].info.averageGPA = parseFloat((totalGPA / totalSections).toFixed(3));
        }
    }
    return existingData;
}
