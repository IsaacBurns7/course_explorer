...
function transformParsedData(parsedRows, year = 2025, semester = "Spring") {
    ...
    parsedRows.forEach(row => {
        const [dept, courseNum, section, numA, pctA, numB, pctB, numC, pctC, numD, pctD, numF, pctF, total, gpa, I, S, U, Q, X, studentCount, profName] = row;
...
function enrichSectionsWithAPI(courseData, apiResponse) {
    for (const entry of apiResponse) {
        ...
        if (!targetSection) {
            ...
            if (!sectionsByTerm[termKey])
                sectionsByTerm[termKey] = [];
            targetSection = {
                section: sectionNum
            };
        ...
        // --- Set extra fields
        let attributes = [];
        if (entry.SWV_CLASS_SEARCH_ATTRIBUTES)
            attributes = entry.SWV_CLASS_SEARCH_ATTRIBUTES
                .split("| ");
        ...
        for (const dept of departments) {
            ...
            if (isSemesterOver(`${semester} ${year}`)) {
                ...
                for (const entry of json) {
                    if (entry.SWV_CLASS_SEARCH_SUBJECT !== dept)
                        continue;
...
async function getPrereq(courseData) {
    ...
    for (const course of Object.keys(courseData)) {
        ...
        if (!maxSemester)
            continue; // no valid semester found
