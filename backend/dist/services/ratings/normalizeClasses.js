...
for (const prof of professors) {
    ...
    for (const courseCode of Object.keys(courses)) {
        ...
        if (newCode !== courseCode) {
            if (newCourses[newCode]) {
                ...
                newCourses[newCode].averageRating = averageRating(newCourses[newCode].averageRating || 0, newCount, courses[courseCode].averageRating || 0, oldCount);
            }
            else {
                // Rename to PBSI
    ...
    for (const { oldCourse, newCourse } of mapping) {
        ...
        if (existing) {
            ...
            existing.averageRating = averageRating(existing.averageRating || 0, newCount, oldData.averageRating || 0, oldCount);
        }
        else {
