 ...
 *
 * - Finds the prereqs for a course from the course_prereqs SQL table.
 *
 * - If not found/there was an error in fetching the data, it will return false
 */
async function getPrereqBucket(courseName) {
    try {
        const result = await pool.query("SELECT prereqs FROM course_prereqs WHERE course_id = $1", [courseName]);
        if (result.rows.length === 0) {
            return false;
        }
        return result.rows[0].prereqs;
    }
    catch (e) {
        console.error("Error fetching prereq from DB:", e);
        return false;
    }
}
/**
