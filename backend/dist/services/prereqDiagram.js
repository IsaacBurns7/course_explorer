...
function parsePrereqs(input) {
    if (typeof input === "string")
        return { type: "single", id: makeId(), course: input };
...
function gradeMeets(required, got) {
    if (!required)
        return true; // no grade requirement
    if (!got)
        return false; // no grade earned = no pass
...
function evaluateTree(node, taken = [], enrolled = []) {
    ...
    const evaluatedChildren = node.children.map((child) => evaluateTree(child, taken, enrolled));
    let status;
    if (node.type === "and") {
