...
const { isMissingTableError, listProgramsLocal, getProgramRequirementsLocal, } = require("./programsLocalFallback");
let _warnedLocal = false;
function warnLocal() {
    if (_warnedLocal)
        return;
    _warnedLocal = true;
    console.warn("[programs] course_explorer.program_requirements not found — serving program data " +
