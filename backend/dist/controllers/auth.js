...
async function findOrCreateUserFromGoogle(profile) {
    ...
    const existing = await pool.query("SELECT id, email, name, picture FROM course_explorer.users WHERE google_id = $1", [googleId]);
    if (existing.rows.length > 0) {
        return existing.rows[0];
    }
    const inserted = await pool.query(`INSERT INTO course_explorer.users (google_id, email, name, picture)
        VALUES ($1, $2, $3, $4)
        RETURNING id, email, name`, [googleId, email, name, picture]);
    await pool.query(`
...
async function getPlanner(req, res) {
    ...
    if (!token)
        return res.json({});
    const user = jwt.verify(token, process.env.JWT_SECRET);
    const existing = await pool.query("SELECT planner FROM course_explorer.users_planner WHERE id = $1", [user.id]);
...
async function savePlanner(req, res) {
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            await pool.query("UPDATE course_explorer.users_planner SET planner = $1 WHERE id = $2", [compressed, user.id]);
        }
        else {
            await pool.query(`INSERT INTO course_explorer.users_planner (id, planner)
                VALUES ($1, $2)`, [user.id, compressed]);
        }
