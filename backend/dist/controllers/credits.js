...
const { isMissingTableError, listMethodsLocal, listExamsLocal, getExamLocal, } = require("./creditsLocalFallback");
let _warnedLocal = false;
function warnLocal() {
    if (_warnedLocal)
        return;
    _warnedLocal = true;
    console.warn("[credits] course_explorer.credit_equivalency not found — serving credit data " +
...
function evaluateExam(exam, score) {
    ...
    for (const tier of exam.tiers || []) {
        if (tier.score == null)
            continue;
        if (minScore == null || tier.score < minScore)
            minScore = tier.score;
        if (!Number.isNaN(numericScore) && numericScore >= tier.score) {
            if (!best || tier.score > best.score)
                best = tier;
        }
    }
    if (!best) {
...
async function methodRowFromDb(client, method) {
    const result = await client.query(`SELECT method, name, exams FROM course_explorer.credit_equivalency WHERE method = $1`, [method]);
...
async function enrichCourses(client, courses) {
    ...
    const byId = {};
    try {
        const result = await client.query(`SELECT c.id, c.title,
        ...
        for (const row of result.rows)
            byId[row.id] = { title: row.title, hours: row.hours };
    }
    catch (error) {
...
const listMethods = async (req, res) => {
    ...
    try {
        const result = await client.query(`SELECT method AS id, name FROM course_explorer.credit_equivalency ORDER BY name`);
...
const listExams = async (req, res) => {
    ...
    try {
        ...
        if (!row)
            return res.status(404).json({ error: `unknown method '${req.params.method}'` });
    ...
    try {
        ...
        try {
            ...
            if (!row)
                ...
                examRecord = (row.exams || []).find((e) => e.exam.trim().toLowerCase() === target);
        }
        catch (error) {
            if (!isMissingTableError(error))
                throw error;
            warnLocal();
            if (listExamsLocal(method) === null)
                unknownMethod = true;
            else
                examRecord = getExamLocal(method, exam);
        }
        if (unknownMethod)
            return res.status(404).json({ error: `unknown method '${method}'` });
        if (!examRecord)
            return res.status(404).json({ error: `unknown exam '${exam}'` });
